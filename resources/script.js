...
// Add Food Form
	$('#add_food_form').click( function(){
        // Large HTML prepended below
		$('#food-forms-cnt').prepend('<label for="food_name">Food name:</label> <input class="input-textarea" type="text" id="food_name" name="food_name['
...
// This function gets suggestions for food
function autocomplet() {
	var food_input = $('#food_search').val();
	if (food_input.length > 0) {
	    $.ajax(
	    {
	        url: '../../resources/ajax/auto_complete_food.php',
	        type: 'POST',
...
// Run autocomplete function when user finishes typing a key
$('#food_search').keyup( function(){
	autocomplet();
});

// This delegated function puts the clicked suggestion into the text box,
// and populates a prepended form with the data
$('body').on('click', '.food_search_item', function(){
    var text = $(event.target).parent().text();
	$('#food_search').val(text);
	$('#food_search_list').hide();
// Do ajax to get data to populate form
    $.ajax(
    {
        url: '../../resources/ajax/get_food.php',
        type: 'POST',
        data: {text:text},
        cache: false,
        success: function(data)
        {
        	// Store variables to store below
        }
    });
    // Large HTML prepended below
	$('#food-forms-cnt').prepend('<label for="food_name">Food name:</label> <input class="input-textarea" type="text" id="food_name" name="food_name['
